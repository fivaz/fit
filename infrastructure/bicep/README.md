...
The API's custom domain (`api-fit-<env>.sfivaz.com`) has **no Bicep parameter at all** — it's bound purely imperatively in `azure-deploy.yml`, for the same underlying reason as the frontend.

**Why neither custom domain is ever set declaratively in Bicep:** ARM replaces the Container App's `ingress` object **wholesale** on every deployment — even a Bicep template that never mentions `customDomains` at all still clears an existing binding, because omission and explicit-empty are indistinguishable to ARM for this property (confirmed via `az deployment group what-if`). A live custom domain binding, once established, can only survive across deploys by being reasserted outside Bicep every time. So:
...
2. The same run's "Bind API custom domain" step attempts `hostname add` + `hostname bind`. **This frequently does not succeed on the first attempt:**
   ...
   - Once DNS is correct, give it the full time Azure warns about ("up to 20 minutes") before assuming it's stuck.
...
- **Cutover is two commands, not one:** `az containerapp revision label add` only tags a revision with the `production` label; it does **not** move traffic weight onto it. `az containerapp ingress traffic set --label-weight production=100` is the step that actually cuts traffic over. Both `azure-deploy.yml`'s dev step and the `containerapp-bluegreen-cutover` action always run label-add followed by traffic-set — never one without the other.
- **Staging/prod:** the `containerapp-bluegreen-cutover` composite action smoke-tests the new revision's `/api/health` directly at its own FQDN, and only then runs the label-add + traffic-set pair — the actual cutover. A failed smoke test leaves live traffic untouched on the old revision.
- **Dev:** skips the smoke-test gate and cuts over immediately after every deploy, for fast disposable iteration.
- **Rollback:** re-run both commands (label add, then traffic set) pointed at the previous revision name — no rebuild, seconds not minutes.
