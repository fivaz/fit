...
- **Cutover is two commands, not one — this was learned by shipping the gap live:** `az containerapp revision label add` only tags a revision with the `production` label; it does **not** move traffic weight onto it. The first real deploy through this pipeline showed exactly that — the label moved to the new revision, but the old one kept serving 100% of traffic until `az containerapp ingress traffic set --label-weight production=100` ran immediately after. Both `azure-deploy.yml`'s dev step and the `containerapp-bluegreen-cutover` action now always run label-add followed by traffic-set — never one without the other.
