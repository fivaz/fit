...
### Blue-Green Deployment (staging/prod) — work in progress, not yet active

The intended design: `container-apps.bicep` in `activeRevisionsMode: 'Multiple'`, with ingress traffic pinned to whichever revision holds a `production` label rather than whatever deployed most recently, so a new revision (each deploy passes `revisionSuffix`, the short git SHA, giving it its own directly addressable FQDN) can be smoke-tested with zero live traffic before cutover.

**Currently reverted to `traffic: [{ latestRevision: true, weight: 100 }]`** — routing to the newest revision immediately, the pre-blue-green behavior — because a label-targeted traffic rule needs some revision to already hold that label to resolve against, and nothing bootstraps the `production` label onto a revision before the first label-based deploy would need it (the same class of chicken-and-egg problem as the custom domain bindings above). The `containerapp-bluegreen-cutover` composite action (smoke-test-then-label-move) still exists and is still wired into `azure-deploy.yml` for staging/prod, but moving a label that nothing routes traffic by currently has no effect — it needs the ingress traffic rule restored plus a solved bootstrap sequence before it's actually providing the zero-downtime guarantee. Until then, staging/prod migrations should still be treated as expand/contract-safe going forward, since re-enabling this is meant to land without a database-compatibility scramble.
