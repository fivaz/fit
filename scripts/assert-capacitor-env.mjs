...
/** Mirrors `resolveEnvUrl` in `lib/env/mobile-dev-url.ts`. */
function resolveEnvUrl(specific) {
	const trimmed = specific?.trim();
	if (trimmed) return trimmed;
	const mobileDevUrl = process.env.MOBILE_DEV_URL?.trim();
	return mobileDevUrl || undefined;
}
...
export function assertCapacitorEnv() {
	const apiBaseUrl = resolveEnvUrl(process.env.NEXT_PUBLIC_API_BASE_URL);
	const authBaseUrl = resolveEnvUrl(process.env.NEXT_PUBLIC_AUTH_BASE_URL);
	...
	const errors = [
		...
		validateHttpUrl("NEXT_PUBLIC_AUTH_BASE_URL", authBaseUrl, [
